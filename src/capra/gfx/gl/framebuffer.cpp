...
Framebuffer::Framebuffer(GladGLContext &gl) : gl(gl) {}
...
void Framebuffer::del_id_() {
  if (id != 0) {
    CAPRA_LOG_DEBUG("Deleting framebuffer ({})", id);
    for (auto &p: tex_attachments_) {
      CAPRA_LOG_DEBUG("FBO/Deleting texture ({} / {})", p.second, id);
      gl.DeleteTextures(1, &p.second);
    }
    for (auto &p: rbo_attachments_) {
      CAPRA_LOG_DEBUG("FBO/Deleting renderbuffer ({} / {})", p.second, id);
...
FramebufferBuilder::FramebufferBuilder(GladGLContext &gl, GLsizei width, GLsizei height)
    : gl(gl), width_(width), height_(height) {
  gen_id_();
...
FramebufferBuilder &FramebufferBuilder::texture(const std::string &tag, TexFormat internalformat, bool retro) {
  ...
  CAPRA_LOG_DEBUG("FBO/Generated texture ({})", tex);
...
FramebufferBuilder &FramebufferBuilder::renderbuffer(const std::string &tag, RBufFormat internalformat) {
  ...
  CAPRA_LOG_DEBUG("FBO/Generated renderbuffer ({})", rbo);
...
std::unique_ptr<Framebuffer> FramebufferBuilder::check_complete() {
  if (gl.CheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
    ...
    CAPRA_LOG_DEBUG("Completed framebuffer ({})", id_);
  gl.BindFramebuffer(GL_FRAMEBUFFER, 0);
...
void FramebufferBuilder::gen_id_() {
  ...
  CAPRA_LOG_DEBUG("Generated framebuffer ({})", id_);
}
...
GLenum FramebufferBuilder::get_texture_format(GLenum internalformat) {
  switch (internalformat) {
    ...
    default:
      CAPRA_LOG_ERROR("Bad value given to get_texture_format: {}", internalformat);
      return GL_RGBA;
  }
}
