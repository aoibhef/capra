...
InputMgr::InputMgr(const Engine &engine) : Module(engine, ModuleTag::InputMgr) {
  ...
  MsgBus::subscribe(msg_endpoint_id_, MsgType::GlfwMouseButton);
  MsgBus::subscribe(msg_endpoint_id_, MsgType::GlfwScroll);
...
void InputMgr::received_msg_(const Msg &msg) {
  std::visit(overloaded {
      ...
      [&](const GlfwCursorEnter &e) {
        mouse.entered = e.entered == 1;
      },

      [&](const GlfwMouseButton &e) {
